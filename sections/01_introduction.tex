...
\caption{Activity diagram of Experiment~1. Nine encodings are evaluated with LPQ and SVM using participant-separated outer folds, and the leading encoding is then used to compare classifiers (LR denotes logistic regression and RF random forest). The attribution branch drawn in the diagram is not reported in this paper. Experiments~2 and~3 reuse the same participant partitions (Sections~\ref{sec:nested-followup} and~\ref{sec:competitive}). The tablet and stylus are schematic.}
\label{fig:activity}
\end{figure*}
\noindent
...
This initial study evaluates the representation in three experiments that share participant-separated partitions. Experiment~1 adds speed, pressure, altitude and azimuth to the static trace, one at a time and as RGB triplets, and compares classifiers on the leading encoding (Figure~\ref{fig:activity}). Experiment~2 selects the encoding with training participants only and displaces the signals along each stroke to test whether their position matters. Experiment~3 compares the selected minimaps with a kinematic/pressure descriptor and pretrained ResNet18 features on the repeated \textit{l}, \textit{le} and \textit{les} tasks examined in earlier work on PaHaW \cite{Casademunt2023}.

The results indicate that the classifiers make use of the encoded dynamics. Training-only selection preferred a dynamic encoding over the static trace in almost every training set, displacing the signals along the stroke lowered performance, and on the three repeated-pattern tasks the minimap pipeline gave the best observed participant-level decisions among the four methods. A cohort of 75 participants leaves the size of these gains uncertain; the study establishes how pen dynamics can be carried into an image and where they help, rather than delivering a clinical tool.
