...
% The current main manuscript precedes this gallery in the editorial PDF.
% build_paper.py updates gallery_start.tex after compiling main.tex; the same
% file is included in the Overleaf package for standalone gallery compilation.
...
\begin{document}
\setcounter{page}{\galleryfirstpage}
...
\newpage
\gallerypage{A13. Competitive methods on the three fixed tasks}{competitive_task_profiles}
{Task-specific macro F1 on T2/T3/T4, the set fixed from the Casademunt thesis before fitting the competitive baselines. Values average five repetitions after pooling participant-separated outer folds. Encodings and classifier parameters are selected within training. The lines aid reading across three categorical tasks; this descriptive panel does not establish task-specific significance.}
{An alternative to the aggregate competitive-method figure when the profiles of the three repeated-pattern tasks are of interest.}
